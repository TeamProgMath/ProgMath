...
namespace RegArchLib {
	...
	*/
		...
		:cAbstCondVar(eFigarch)  // call constructor of cAbstCondVar with type eFigarch
	{
		...
		for (register uint i = 0; i < myNParam-1; i++)
			mvGradPolArch.at(i) = cPolynome(mvNTruncLag);
	...
	*/
	...
	{
		cFigarch* myFigarch = new cFigarch(mvConst, mvArch, mvGarch, mvFracD, mvNTruncLag);
		return myFigarch;
	}
...
//			throw cError("cFigarch::ReAlloc - theNumParam must be in 1, 2.");
	...
	*/
	...
	{
		if (GetCondVarType() == eNotKnown)
		{
			mvArch = cDVector(theSrc.mvArch);
			mvGarch = cDVector(theSrc.mvGarch);
			mvPolArch = cPolynome(theSrc.mvPolArch);
		...
		for (uint i = 0; i < myNParam-1; i++)
			mvGradPolArch.at(i) = theSrc.mvGradPolArch.at(i);
