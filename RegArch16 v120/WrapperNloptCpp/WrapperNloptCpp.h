...
{
	class _DLLEXPORT_ cNLOPTResult
	{
	public :
		uint			mDimension;
		bool			mConvergence;
	...
	class _DLLEXPORT_ cNLOPTAlgoParam
	{
	public :
	...
	public :
		cNLOPTAlgoParam();
		cNLOPTAlgoParam(nlopt_algorithm theAlgo, bool theMinimisation=true, double theStopValue=1e200, double thefTol=1e-3, double thexTol = 1e-3, uint theMaxFuncEval=INT_MAX, double theComputeTimeMax=60);
		virtual ~cNLOPTAlgoParam();

	}; // class cNLOPTAlgoParam
	

	class _DLLEXPORT_ cNloptWrapperCpp
	{
	private:
		nlopt_opt mvOpt;
