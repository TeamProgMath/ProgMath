...
namespace WrapperNloptCppNamespace {
	...
	cNLOPTAlgoParam::cNLOPTAlgoParam(nlopt_algorithm theAlgo, bool theMinimisation, double theStopValue, double thefTol, double thexTol, uint theMaxFuncEval, double theComputeTimeMax)
	{
		this->mAlgo = theAlgo;
		this->mfTol = thefTol;
	...
	cNLOPTResult::cNLOPTResult(uint theDimension)
	{
		...
		mXOptim = (double *)calloc(theDimension, sizeof(double));
		mConvergence = false;
		mFunctValue = 0;
		mConvergenceStatus = NLOPT_FAILURE;
		sprintf(mConvergenceString, "");
	...
	double* mylb = (double *)calloc(theDimension, sizeof(double));
	double* myub = (double *)calloc(theDimension, sizeof(double));
	...
	void cNloptWrapperCpp::Optimize(double* theInitPoint, uint theDimension, cNLOPTAlgoParam& theAlgoParam, cNLOPTResult& theResult)
	{
		if (theResult.mXOptim == NULL)
			theResult.mXOptim = (double *)malloc(theDimension*sizeof(double));
		for (register uint i = 0; i < theDimension; i++)
	...
	clock_t myStart = clock();
		...
		if ((theResult.mConvergenceStatus >= NLOPT_SUCCESS) && (theResult.mConvergence < NLOPT_MAXEVAL_REACHED))
