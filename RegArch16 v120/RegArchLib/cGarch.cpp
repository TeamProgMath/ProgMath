				 ...
				 */
					...
					void cGarch::ComputeHess(uint theDate, const cRegArchValue& theData, cRegArchGradient& theGradData, cRegArchHessien& theHessData, cAbstResiduals* theResiduals)
					{
						...
						for (i = 0; i < theHessData.mCurrentHessVar.GetNCol(); i++)
							for (j = 0; j <= MIN(myp, theDate); j++)
								theHessData.mCurrentHessVar[i][1 + j] -= 2 * theData.mUt[theDate - j] * theGradData.mGradMt[j][i];
						//GARCH
						myp = mvGarch.GetSize();
						for (i =0; i < MIN(myp, theDate); i++)
							for (j = 0; j < theHessData.mCurrentHessVar.GetNCol(); j++)
								theHessData.mCurrentHessVar[mvArch.GetSize() + 1+i][j] +=  theGradData.mGradHt[i][j];
						for (i = 0; i < theHessData.mCurrentHessVar.GetNCol(); i++)
							for (j =0 ; j < MIN(myp, theDate); j++)
								theHessData.mCurrentHessVar[i][mvArch.GetSize() + 1+j] +=   theGradData.mGradHt[j][i];

						for (i = 0; i < MIN(myp, theDate); i++)
							theHessData.mCurrentHessVar += mvGarch[i] * theHessData.mHessHt[i];
					}
